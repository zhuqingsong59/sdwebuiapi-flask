...
class WebUIApi:
    ...
    def _to_api_result(self, response):
        ...
        return WebUIApiResult(images, parameters, info, poses)
    ...
    async def async_post(self, url, json):
        ...
        async with aiohttp.ClientSession() as session:
            auth = aiohttp.BasicAuth(
                self.session.auth[0], self.session.auth[1]) if self.session.auth else None
    ...
    def util_get_current_model(self):
        ...
        if ("sd_model_checkpoint" in options):
            ...
            sd_model = [model for model in sd_models if model["sha256"]
